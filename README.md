...
- Clean up was done by transforming the data on the tables to match the correct values
  - The ID columns were changed to text from whole numbers
...
- From the date data on the visits table, I created a Visit_dates dimension table
  - The duplicate dates were removed and the date column was sorted; oldest to newest date
...
- From the Pharmacy transactions drug name data, I created a Drug dimension table
  ...
  - On the pharmacy transactions table, I proceeded to create merge queries to have the Drug_ID column showing on the Pharmacy transactions fact table
...
- Star schema was used containing 2 fact tables and several dimension tables
